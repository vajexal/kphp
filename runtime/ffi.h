...
template<class T>
class_instance<C$FFI$CData<T>> ffi_clone(const C$FFI$CDataRef<T> &ref) {
...
public:
  ...
  bool is_shared_lib_loaded(int id);

  void load_shared_lib(int id);
...
// This FFI object is shared among all workers.
// Its methods are not synchronized; it's up to the caller
// to use the proper synchronization to avoid data races.
...
template<class T>
int64_t f$FFI$$sizeof(const C$FFI$CDataRef<T> &ref) {
...
// TODO: make ffi_cast alloc-free somehow? We could pass cast'ed types
// as CDataRef<typename std::remove_pointer<ToType>::type> somehow,
// but that would require some changes into the way we infer types for it
template<class ToType, class FromType>
class_instance<C$FFI$CData<ToType>> ffi_cast(FromType &v) {
  auto cdata = ffi_new_cdata<ToType>();
  cdata->c_value = reinterpret_cast<ToType>(&v);
  return cdata;
}
...
template<class T>
C$FFI$CDataRef<T> ffi_c2php_ref(T &v) {
  return C$FFI$CDataRef<T>{&v};
}
