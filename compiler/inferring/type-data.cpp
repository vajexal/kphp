    ...
    case tp_Class: {
      ...
      if (auto *as_ffi = klass->ffi_class_mixin) {
        if (as_ffi->is_ref()) {
          ...
          res += "CDataPtr<" + G->get_ffi_root().get_ffi_type(klass)->to_decltype_string() + ptr + ">";
        } else {
          // TODO: can we avoid manual ptr addition here?
          res += "class_instance<C$FFI$CData<" + G->get_ffi_root().get_ffi_type(klass)->to_decltype_string() + ">>";
        }
        break;
      }
      res += "class_instance<";
